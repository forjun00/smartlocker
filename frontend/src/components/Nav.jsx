  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontFamily: "'Space Mono', monospace", fontSize: 11, letterSpacing: '0.12em', color: '#6E6880',
      }}>
        <LockIcon open={false} size={13} />
        SMARTLOCKER
      </div>
      {!lockerMatch && <button style={pill(isAdmin)} onClick={() => navigate('/')}>ADMIN GRID</button>}
      {lockerMatch && (
        <button style={pill(true)}>SLOT {slotLabel}</button>
      )}
    </div>
  )
}
